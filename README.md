...
Wiring is a build-time reading: `NEXT_PUBLIC_DOOR_OPEN_VALUE` (default `1`: a door contact is ON while open), `NEXT_PUBLIC_PANIC_PRESSED_VALUE` (default `0`: the buttons are normally-closed, so OFF means pressed) and `NEXT_PUBLIC_PHASE_OK_VALUE` (default `1`).

The plant collector can read the panel's own SET LOW / SET HIGH columns (`RECORD_SETPOINTS=true`); they arrive as `"<room> Set Low"` / `"<room> Set High"` rows and fold into that room's limits on both paths. Two panel habits are handled: a room left at `0 / 0` is treated as having no limits (the operator file may fill it), and a room whose low was typed above its high is shown and evaluated as the band the other way round (LOW −14 / HIGH −25 becomes −25 to −14), with a ⇄ mark after the range on the card.

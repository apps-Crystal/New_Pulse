...
// computed from the 5-minute rows and the per-input door / panic / phase summaries.
export const dynamic = 'force-dynamic';
export const maxDuration = 30;
export const revalidate = 0;
export const runtime = 'nodejs';

import { getDailySamples, getDailyDoors } from '../../../../lib/db';
import { buildZoneReports, buildInputsSummary } from '../../../../lib/reports';
...
export async function GET(request) {
  ...
  try {
    const [samples, doors] = await Promise.all([getDailySamples(day, zone === 'all' ? null : zone), getDailyDoors(day)]);
    const reports = buildZoneReports(day, samples, { doors });
    ...
    const inputs = zone === 'all' ? buildInputsSummary(doors, reports) : null;
    const bytes = await renderDayPdf({ day, reports, inputs, logoPng: Buffer.from(LOGO_PNG_BASE64, 'base64'), generatedAt: new Date() });

...
import { getDailyReports, getDailySamples } from '../../../../lib/db';
...
export async function GET(request) {
  ...
  try {
    const all = await getDailyReports(day);
    const reports = zone === 'all' ? all : all.filter((r) => r.zone_id === zone);
    if (reports.length === 0) return Response.json({ ok: false, error: `no archived report for ${day}${zone === 'all' ? '' : ` / ${zone}`}` }, { status: 404, headers: NO_STORE });
    const samplesByZone = await getDailySamples(day, zone === 'all' ? null : zone);
    const bytes = await renderDayPdf({ day, reports, samplesByZone, logoPng: Buffer.from(LOGO_PNG_BASE64, 'base64'), generatedAt: new Date() });

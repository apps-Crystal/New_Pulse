...
// nobody configured: no limits (the operator file may still fill them). Low above high is a data-entry
// slip on the panel: keep the numbers so the operator sees exactly what the panel says, flag the room,
// and never raise a temperature alarm from it.
function normaliseLimits(room) {
  const lo = room.setLow;
  const hi = room.setHigh;
  room.limitsInvalid = false;
  if (lo == null || hi == null) return;
  if (lo === hi) { room.setLow = null; room.setHigh = null; return; }

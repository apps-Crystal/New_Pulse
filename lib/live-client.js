 ...
 *   getSeed()             readings [{ tag, value, ts }] from the last database snapshot, used once to fill
 *                         the rooms the first broadcast does not carry
 */
...
function connectSupabase({ onSnapshot, onStatus, getSetpoints, getStaleMs, getSeed }) {
  const state = snap.createLiveState();
  // The collector broadcasts one panel screen at a time, so the first message carries only half the
  // rooms. Seed the browser's state once from the last database snapshot (with the database's own
  // timestamps, so the OFFLINE rule still applies) rather than blanking the other half for a rotation.
  let seeded = false;
  const seedOnce = (now) => {
    if (seeded) return;
    seeded = true;
    if (!getSeed) return;
    try { snap.ingest(state, getSeed() || [], now); } catch { /* a bad seed must not block live data */ }
  };
  let closed = false;
  let channel = null;
  let retryTimer = null;
  ...
  const join = () => {
    ...
    ch.on('broadcast', { event: 'readings' }, (msg) => {
      ...
      const now = Date.now();
      seedOnce(now);
      // Newest timestamp per tag wins, so the fresh broadcast always overrides a seeded value.
      const n = snap.ingest(state, snap.normalizeReadings(msg && msg.payload), now);
      const x = snap.ingestExtra(state, msg && msg.payload, now);
      if (!n && !x) return;
      const { rooms, stats, inputs, panelAlarms } = snap.buildRooms(state, { now, staleMs: staleMs(), setpoints: getSetpoints ? getSetpoints() : {} });
      onSnapshot({
        ok: true,
        connected: true,
        source: 'live',

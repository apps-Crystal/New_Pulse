...
/** Minutes as "45 s", "12.5 min" or "1 h 4 min". */
...
function summaryPage(ctx, zr) {
  ...
  // Doors: how often and how long, ok versus long ("concerning") openings.
  const doors = Array.isArray(zr.doors) ? zr.doors : [];
  if (doors.length) {
    const yD = title(sh, M, y, 'Doors') - 1;
    const rows = doors.map((d) => [
      d.name,
      `opened ${d.opens} times, open ${fmtMin(d.openMinutes)} in total · ${d.okOpens} short (ok) · ${d.longOpens} long (${fmtMin(d.longMinutes)}; longest ${fmtMin(d.longestSec / 60)})` +
        `${d.openAtStart ? ' · open at 00:00' : ''}${d.openAtEnd ? ' · still open at midnight' : ''}`,
    ]);
    y = kvBox(sh, M, yD, CW, rows, { labelW: 52 }) + 6;
  }
  ...
  const longOpenings = doors.flatMap((d) => d.longEvents.map((e) => ({ ...e, door: d.name, thresholdSec: d.thresholdSec })));
  for (const e of longOpenings.slice(0, 4)) notes.push(`${e.door} open too long: ${fmtHM(e.at, off)} - ${e.until ? fmtHM(e.until, off) : 'midnight'} (${fmtMin(e.minutes)}${e.ongoing ? ', still open' : ''}); the limit is ${fmtMin(e.thresholdSec / 60)}.`);
  if (longOpenings.length > 4) notes.push(`... and ${longOpenings.length - 4} more long opening(s).`);
...
function coverPage(ctx, reports) {
  ...
  const cols = [['Zone', 84, 'left'], ['Band', 62, 'left'], ['Lowest', 36, 'right'], ['Highest', 38, 'right'], ['Average', 38, 'right'], ['SD', 26, 'right'], ['MKT', 32, 'right'], ['Lower', 34, 'center'], ['Upper', 34, 'center'], ['Readings', 38, 'right'], ['No data', 32, 'right'], ['Doors', 36, 'right'], ['Long', 33, 'right']];
  const rowH = 15;
  sh.rect(M, y, CW, rowH, { fill: C.stripe, stroke: C.box, width: 0.6 });
  let cx = M;
  for (const [name, w, align] of cols) { sh.text(name, cx + 3, y + 3.9, { size: 7, font: 'bold', align, width: w - 6 }); cx += w; }
  let ry = y + rowH;
  reports.forEach((zr, i) => {
    const { zone, stats: s } = zr;
    const doors = Array.isArray(zr.doors) ? zr.doors : [];
    const opens = doors.reduce((n, d) => n + d.opens, 0);
    const longOpens = doors.reduce((n, d) => n + d.longOpens, 0);
    if (i % 2 === 1) sh.rect(M, ry, CW, rowH, { fill: C.stripe });
    const vals = [zone.label, zone.limits ? `${num(zone.limits.low, 0)} to ${num(zone.limits.high, 0)} °C` : '—', num(s.lowest), num(s.highest), num(s.average), num(s.stdDev), num(s.mkt), null, null, String(s.readings), s.gapMinutes ? `${s.gapMinutes} min` : '—', doors.length ? String(opens) : '—', doors.length ? String(longOpens) : '—'];
    cx = M;
    cols.forEach(([, w, align], ci) => {
      if (ci === 7) badge(sh, cx + (w - 30) / 2, ry + 1.5, s.lower.status, { w: 30, h: 12 });
      else if (ci === 8) badge(sh, cx + (w - 30) / 2, ry + 1.5, s.upper.status, { w: 30, h: 12 });
      else sh.text(vals[ci], cx + 3, ry + 3.9, { size: 7.2, align, width: w - 6, maxWidth: w - 6, font: ci === 0 ? 'bold' : 'reg', color: ci === 12 && longOpens ? C.fail : C.ink });
      cx += w;
    });
    ry += rowH;
  });
  ...
  const inputs = ctx.inputs || null;
  const doorLines = [];
  if (inputs) {
    const lo = inputs.longOpenings || [];
    doorLines.push(lo.length
      ? `Long door openings (10 min or more): ${lo.length} — ${lo.slice(0, 6).map((e) => `${e.zone} ${e.door} ${fmtHM(e.at, ctx.offsetMin)}-${e.until ? fmtHM(e.until, ctx.offsetMin) : 'midnight'} (${fmtMin(e.minutes)})`).join('; ')}${lo.length > 6 ? `; and ${lo.length - 6} more` : ''}.`
      : 'No door was open for 10 minutes or more on this day.');
    const panic = inputs.panic || [];
    const presses = panic.reduce((n, d) => n + d.opens, 0);
    doorLines.push(presses
      ? `Panic buttons: ${presses} press(es) — ${panic.filter((d) => d.opens).map((d) => `${d.tag} ${d.opens}x, ${fmtMin(d.openMinutes)}`).join('; ')}.`
      : `Panic buttons: none pressed${panic.length ? '' : ' (no data)'}.`);
    const phase = inputs.phase || [];
    const faults = phase.reduce((n, d) => n + d.opens, 0);
    doorLines.push(faults ? `Phase preventer: ${faults} fault(s), ${fmtMin(phase.reduce((n, d) => n + d.openMinutes, 0))} in total.` : `Phase preventer: no fault${phase.length ? '' : ' (no data)'}.`);
  }
  const lines = [
    ...
    ...doorLines,
    'Each zone follows on its own pages: summary and chart with its doors, the 5-minute readings table, and a statistics page.',
  ];
 ...
 */
async function renderDayPdf({ day, reports, inputs = null, logoPng = null, generatedAt = new Date(), site = DEFAULT_SITE, offsetMin = DEFAULT_OFFSET_MIN }) {
  ...
  const ctx = { doc, fonts, logo, generatedAt, site, day, offsetMin, inputs };
...
module.exports = { renderDayPdf, reportFileName, fmtElapsed, fmtDT, fmtLongDay, fmtMin, clean, compactEvents, COMPANY, DEFAULT_SITE, DEFAULT_OFFSET_MIN };

...
function applySetpoints(rooms, setpoints) {
  ...
  for (const [id, room] of entries) {
    ...
    if (entry) {
      if (room.setLow == null && entry.setLow != null && Number.isFinite(entry.setLow)) room.setLow = entry.setLow;
      if (room.setHigh == null && entry.setHigh != null && Number.isFinite(entry.setHigh)) room.setHigh = entry.setHigh;
    }

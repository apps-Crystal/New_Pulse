...
function applyRoomSettings(rooms, settings) {
  ...
  for (const [id, room] of entries) {
    ...
    // A working room whose sensor is broken: its temperature cannot be trusted, so no temperature
    // alarm from it (door alarms still apply); the note is shown on the card.
    room.sensorFault = Boolean(entry && entry.sensorFault);
    room.note = entry && entry.note ? String(entry.note) : null;
    if (!operational || room.sensorFault) room.alarm = false;
  }
  return rooms;
}

...
const { applyFixedLimits } = require('./limits');
...
function ingestExtra(state, payload, receivedAt) {
  ...
  if (payload && Array.isArray(payload.inputs)) {
    for (const i of payload.inputs) {
      ...
      // `since` is when the input last changed (the door timer's start); keep ours if the sender has none
      const since = i.since != null ? toMs(i.since, ts) : (prev && prev.value === value ? prev.since : ts);
      state.inputs.set(tag, { value, ts, since });
      accepted += 1;
    }
  }
...
function buildRooms(state, opts) {
  ...
  applyFixedLimits(rooms);
  const stats = snapshotStats(rooms, { now, rowCount: rows.length });
  const inputs = [...state.inputs].map(([tag, r]) => ({ tag, value: r.value, ts: new Date(r.ts).toISOString(), since: new Date(r.since == null ? r.ts : r.since).toISOString() }));

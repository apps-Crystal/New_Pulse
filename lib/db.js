...
async function readLimits(pool, d, cfg, now) {
  if (limitsLib.FIXED_LIMITS_ENABLED) return []; // the plant's bands are fixed: the panel's limit rows are not needed
...
async function readExtras(now) {
  const pool = getPool();
  const out = { inputs: [], panelAlarms: { active: [], recent: [], at: null }, roomSettings: {} };
  try { out.roomSettings = await getRoomSettings(); } catch (err) { noteExtrasError('room_settings', err); }
  try {
    let r;
    try {
      r = await pool.query({ text: `/* inputs */ SELECT DISTINCT ON ("tag") "tag", "value", (EXTRACT(EPOCH FROM "ts") * 1000)::bigint AS ts_ms, (EXTRACT(EPOCH FROM COALESCE("since", "ts")) * 1000)::bigint AS since_ms FROM "public"."panel_inputs" ORDER BY "tag", "ts" DESC`, values: [] });
    } catch (err) {
      if (!(err && err.code === '42703')) throw err; // an older collector without the `since` column
      r = await pool.query({ text: `/* inputs */ SELECT DISTINCT ON ("tag") "tag", "value", (EXTRACT(EPOCH FROM "ts") * 1000)::bigint AS ts_ms, (EXTRACT(EPOCH FROM "ts") * 1000)::bigint AS since_ms FROM "public"."panel_inputs" ORDER BY "tag", "ts" DESC`, values: [] });
    }
    out.inputs = (r.rows || []).filter((x) => x && typeof x.tag === 'string').map((x) => ({ tag: x.tag, value: Number(x.value), ts: msIso(x.ts_ms), since: msIso(x.since_ms) || msIso(x.ts_ms) })).filter((x) => Number.isFinite(x.value));
...
// Per-room settings (in service / out of service), shared by every screen. The table is created on
// first use by the dashboard's own database role and protected like the collector's tables.
const ROOM_SETTINGS_SQL = `/* room_settings */ create table if not exists "public"."room_settings" (
  "zone_id" text primary key, "operational" boolean not null default true, "updated_at" timestamptz not null default now());
alter table "public"."room_settings" add column if not exists "sensor_fault" boolean not null default false;
alter table "public"."room_settings" add column if not exists "note" text;
alter table "public"."room_settings" enable row level security;`;
const ROOM_SETTINGS_SELECT = `/* room_settings */ SELECT "zone_id", "operational", "sensor_fault", "note", (EXTRACT(EPOCH FROM "updated_at") * 1000)::bigint AS updated_ms FROM "public"."room_settings"`;
let roomSettingsReady = null;
function ensureRoomSettings(pool) {
  if (!roomSettingsReady) roomSettingsReady = pool.query({ text: ROOM_SETTINGS_SQL, values: [] }).catch((err) => { roomSettingsReady = null; throw err; });
  return roomSettingsReady;
}
async function getRoomSettings() {
  const pool = getPool();
  await ensureRoomSettings(pool);
  const r = await pool.query({ text: ROOM_SETTINGS_SELECT, values: [] });
  return roomsLib.shapeRoomSettings(r.rows);
}
// setRoomSettings(id, { operational?, sensorFault?, note? }): only the fields given change.
async function setRoomSettings(id, patch) {
  if (!roomsLib.isZoneId(id)) throw new Error(`unknown zone ${id}`);
  const p = patch || {};
  const operational = typeof p.operational === 'boolean' ? p.operational : null;
  const sensorFault = typeof p.sensorFault === 'boolean' ? p.sensorFault : null;
  const note = p.note === undefined ? undefined : (p.note == null || String(p.note).trim() === '' ? null : String(p.note).slice(0, 200));
  const pool = getPool();
  await ensureRoomSettings(pool);
  await pool.query({
    text: `/* room_settings */ insert into "public"."room_settings" ("zone_id", "operational", "sensor_fault", "note", "updated_at")
           values ($1, coalesce($2, true), coalesce($3, false), $4, now())
           on conflict ("zone_id") do update set
             "operational" = coalesce($2, "public"."room_settings"."operational"),
             "sensor_fault" = coalesce($3, "public"."room_settings"."sensor_fault"),
             "note" = case when $5 then $4 else "public"."room_settings"."note" end,
             "updated_at" = now()`,
    values: [id, operational, sensorFault, note === undefined ? null : note, note !== undefined],
  });
  state.cache = null; // the next /api/plc must show the change
  return getRoomSettings();
}
function setRoomOperational(id, operational) {
  return setRoomSettings(id, { operational: Boolean(operational) });
}
...
async function fetchSnapshot() {
  ...
  try {
    ...
    limitsLib.applyFixedLimits(rooms);
    const extras = await readExtras(now);
    roomsLib.applyRoomSettings(rooms, extras.roomSettings);
    ...
    return {
      ...
      inputs: extras.inputs, panelAlarms: extras.panelAlarms, roomSettings: extras.roomSettings,
    };
  } catch (err) {
...
// ---------------------------------------------------------------------------
// Archived days: the 5-minute rows (daily_samples) written nightly by the collector's report agent.
// Everything a report shows is computed from these rows (lib/reports.js); nothing else is stored.
// ---------------------------------------------------------------------------

const REPORT_DAYS_SQL = `/* daily_samples */ SELECT to_char("day", 'YYYY-MM-DD') AS day, count(DISTINCT "zone_id")::int AS zones FROM "public"."daily_samples" GROUP BY "day" ORDER BY "day" DESC LIMIT 400`;
const SAMPLES_SQL = `/* daily_samples */ SELECT "zone_id", "slot", "ts", "avg_temp", "min_temp", "max_temp", "readings", "events" FROM "public"."daily_samples" WHERE "day" = $1::date AND ($2::text IS NULL OR "zone_id" = $2::text) ORDER BY "zone_id", "slot"`;

function missingTable(err) { return err && err.code === '42P01'; }

// Days that have an archive, newest first: [{ day: 'YYYY-MM-DD', zones }]. Empty until the report agent has run.
async function getReportDays() {
  const pool = getPool();
  try {
    const r = await pool.query({ text: REPORT_DAYS_SQL, values: [] });
    return r.rows.map((x) => ({ day: x.day, zones: Number(x.zones) || 0 }));
  } catch (err) {
...
const DOORS_SQL = `/* daily_doors */ SELECT "tag", "zone_id", "kind", "threshold_sec", "rows", "opens", "open_minutes", "ok_opens", "ok_minutes", "long_opens", "long_minutes", "longest_sec", "open_at_start", "open_at_end", "long_events" FROM "public"."daily_doors" WHERE "day" = $1::date ORDER BY "tag"`;

// One day's daily_doors rows (doors, panic buttons, phase preventer). Empty until the report agent has run.
async function getDailyDoors(day) {
  const pool = getPool();
  try {
    const r = await pool.query({ text: DOORS_SQL, values: [day] });
    return r.rows;
  } catch (err) {
    if (missingTable(err)) return [];
    throw err;
...
// One day's daily_samples rows grouped by zone: Map zone_id -> rows sorted by slot. zoneId null = every zone.
async function getDailySamples(day, zoneId = null) {
  const pool = getPool();
  const out = new Map();
  try {
    const r = await pool.query({ text: SAMPLES_SQL, values: [day, zoneId] });
    for (const row of r.rows) {
      if (!out.has(row.zone_id)) out.set(row.zone_id, []);
      out.get(row.zone_id).push(row);
    }
    return out;
  } catch (err) {
    if (missingTable(err)) return out;
    throw err;
  }
}

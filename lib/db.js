...
const ROOM_SETTINGS_SELECT = `/* room_settings */ SELECT "zone_id", "operational", (EXTRACT(EPOCH FROM "updated_at") * 1000)::bigint AS updated_ms FROM "public"."room_settings"`;
...
async function setRoomOperational(id, operational) {
  if (!roomsLib.isZoneId(id)) throw new Error(`unknown zone ${id}`);
  const pool = getPool();
  await ensureRoomSettings(pool);
  await pool.query({
    text: `/* room_settings */ insert into "public"."room_settings" ("zone_id", "operational", "updated_at") values ($1, $2, now())
           on conflict ("zone_id") do update set "operational" = excluded."operational", "updated_at" = now()`,
    values: [id, Boolean(operational)],
  });

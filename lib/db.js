...
// ---------------------------------------------------------------------------
// Archived daily reports (daily_reports / daily_samples, written by the collector's report agent)
// ---------------------------------------------------------------------------

const reportsLib = require('./reports');

const REPORT_DAYS_SQL = `/* daily_reports */ SELECT to_char("day", 'YYYY-MM-DD') AS day, count(*)::int AS zones FROM "public"."daily_reports" GROUP BY "day" ORDER BY "day" DESC LIMIT 400`;
const REPORT_DAY_SQL = `/* daily_reports */ SELECT *, to_char("day", 'YYYY-MM-DD') AS day FROM "public"."daily_reports" WHERE "day" = $1::date`;
...
// One day's daily_reports rows (raw columns), in dashboard zone order.
async function getDailyReports(day) {
  const pool = getPool();
  try {
    const r = await pool.query({ text: REPORT_DAY_SQL, values: [day] });
    return reportsLib.sortReports(r.rows);
  } catch (err) {
    if (missingTable(err)) return [];
    throw err;
  }

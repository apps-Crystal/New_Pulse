...
function classifyInputs(inputs, now) {
  ...
  for (const raw of Array.isArray(inputs) ? inputs : []) {
    ...
    const since = raw.since != null ? toMs(raw.since, ts) : ts;
    let m;
    if ((m = RE_PANIC.exec(tag))) {
      panic.push({ n: Number(m[1]), tag, pressed: value === PANIC_PRESSED_VALUE, value, ts, since });
      ...
      const entry = { tag, label, open: value === DOOR_OPEN_VALUE, value, ts, since, room: m[1].trim() };
...
// doorAlarms(io, now, afterMs) -> doors that have been open for at least afterMs, with their zone and how long.
function doorAlarms(io, now, afterMs) {
  const t = Number.isFinite(now) ? now : Date.now();
  const limit = Number.isFinite(afterMs) ? afterMs : 30000;
  const out = [];
  for (const [zoneId, list] of Object.entries((io && io.doors) || {})) {
    for (const d of list) {
      if (!d.open) continue;
      const openMs = t - d.since;
      if (openMs >= limit) out.push({ zoneId, tag: d.tag, room: d.room, label: d.label, since: d.since, openMs, twoDoors: list.length > 1 });
    }
  }
  out.sort((a, b) => b.openMs - a.openMs); // longest open first
  return out;
}

module.exports = { classifyInputs, doorAlarms, DOOR_OPEN_VALUE, PANIC_PRESSED_VALUE, PHASE_OK_VALUE };

// Pulse live feed hub (server side).
//
// The collector on the plant PC pushes readings over a WebSocket the moment it reads them from the HMI;
// dashboards subscribe on the same endpoint and get a fresh snapshot pushed on every update. The database
// stays the durable record and the fallback: this hub only ever holds the newest reading per room in memory,
// and when it has nothing fresh the dashboard polls /api/plc exactly as before.
//
...
//
// Readings go through rowsToSnapshot() - the same code path the database rows take - so the live view and
// the fallback view can never disagree about zone names, offline rules or alarm limits.
//
// CommonJS on purpose: required by server.js and by Next route handlers. Requiring it never opens a socket;
// `ws` is only loaded inside attach(), so this module is harmless in a serverless bundle (Vercel), where
// there is no long-lived server to attach to and the dashboard simply keeps polling.
...
function freshState() {
  return {
    ...
    limits: new Map(),      // tag -> { setLow, setHigh }  (from "<tag> Set Low/High" readings)
...
// ---------------------------------------------------------------------------
// Ingest + snapshot (pure apart from module state; unit-tested)
// ---------------------------------------------------------------------------

// ingest(readings, receivedAt) -> number of readings accepted.
...
function feedAgeMs(now) {
  return state.lastReadingsAt == null ? null : now - state.lastReadingsAt;
}
...
function buildSnapshot(now) {
  ...
  const rooms = rowsToSnapshot(LIVE_DETECTED, rows, { now: t, staleMs: cfg.staleMs });
  setpointsLib.applySetpoints(rooms, ensureSetpoints());
  const stats = snapshotStats(rooms, { now: t, rowCount: rows.length });
  const live = isLive(t);
  return {
    ok: true,
    connected: live,
    ...
    live: {
      ...
      lastReadingsAt: state.lastReadingsAt ? new Date(state.lastReadingsAt).toISOString() : null,
...
function getLatestSnapshot(now) {
  if (state.latest.size === 0) return null;
...
function attachSubscriber(ws) {
  ...
  if (state.latest.size) send(ws, { type: 'snapshot', ...buildSnapshot(Date.now()) });
}

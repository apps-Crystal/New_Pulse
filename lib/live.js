...
function buildSnapshot(now) {
  const t = Number.isFinite(now) ? now : Date.now();
  const cfg = getConfig();
  const { rooms, stats, inputs, panelAlarms } = snap.buildRooms(state.feed, { now: t, staleMs: cfg.staleMs, setpoints: ensureSetpoints() });
  const live = isLive(t);
  return {
    ok: true,
    connected: live,
...
function onPublisherMessage(ws, data) {
  ...
  if (msg.type === 'readings') {
    const n = ingest(msg.readings, Date.now()) + snap.ingestExtra(state.feed, msg, Date.now());
...
function attach(server, opts) {
  ...
  server.on('upgrade', async (req, socket, head) => {
    ...
    // A dashboard (subscriber) must carry the same Pulse session as every page and API. WebSocket upgrades
    // never pass through the Next.js middleware, so the gate is repeated here; the collector is unaffected
    // (it is a publisher and presents PULSE_LIVE_TOKEN instead).
    if (who.role === 'subscriber' && session.authEnabled()) {
      const cookie = session.parseCookie(req.headers && req.headers.cookie, session.COOKIE_NAME);
      const user = session.configProblem() ? null : await session.verifySession(cookie);
      if (socket.destroyed) return;
      if (!user) {
        log(`dashboard refused from ${socket.remoteAddress}: no valid session`);
        socket.write('HTTP/1.1 401 Unauthorized\r\nConnection: close\r\n\r\n');
        socket.destroy();
        return;
      }
